...
#define _IRSTD_LOG1(message) __IRSTD_LOG(IrStd::Logger::getDefault(), IrStd::Logger::Level::Info, IrStd::Topic::None, message)
#define _IRSTD_LOG2(level, message) __IRSTD_LOG(IrStd::Logger::getDefault(), level, IrStd::Topic::None, message)
#define _IRSTD_LOG3(level, topic, message) __IRSTD_LOG(IrStd::Logger::getDefault(), level, topic, message)
...
#define _IRSTD_LOG_X2(logger, message) __IRSTD_LOG(logger, IrStd::Logger::Level::Info, IrStd::Topic::None, message)
#define _IRSTD_LOG_X3(logger, level, message) __IRSTD_LOG(logger, level, IrStd::Topic::None, message)
#define _IRSTD_LOG_X4(logger, level, topic, message) __IRSTD_LOG(logger, level, topic, message)
 ...
 */
#define __IRSTD_LOG(logger, level, topic, message) \
	if (!logger.isIgnored(level, topic)) \
	{ \
		IRSTD_SCOPE_THREAD(scope, IrStdMemoryNoTrace); \
		IrStd::Logger::Info info{level, topic, __LINE__, (strrchr(__FILE__, '/') ? \
				strrchr(__FILE__, '/') + 1 : __FILE__), __func__}; \
		*(logger).entry(info) << message; \
	}
...
namespace IrStd
{
	...
	public:
		...
		public:
			...
			~Filter()
			{
				std::cerr << "delete " << static_cast<void*>(this) << std::endl;
			}
		...
		OutputStreamPtr entry(Info& info);
